...
/*   Updated: 2022/12/08 05:17:08 by clacaill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		choose_print(const char *format, va_list argp, int *len)
{
	...
	if (!format)
		...
		ft_nbr_prnt(argp, len);
	else if (*format == '%' && *(format + 1) == 'u')
		ft_unsigned_prnt(argp, len);
...
void	ft_nbr_prnt(va_list argp, int *len)
{
	...
	if (nb > 9)
	{
		...
		ft_char_prnt(nb % 10 + '0', len);
	}
	else
		ft_char_prnt(nb + '0', len);
}
